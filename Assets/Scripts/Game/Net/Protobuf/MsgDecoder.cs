...
public JsonData decode(string route, byte[] buf)
{
    ...
    this.offset = 0;
    object proto = null;
    if (this.protos.Keys.Contains(route))
    {
        JsonData jdata = this.protos[route];
        JsonData msg = new JsonData();
        return this.decodeMsg(msg, jdata, this.buffer.Length);
    }
                        ...
                        if (((JsonData)(value)).TryGetValue("option", out option))
                        {
                            switch (option.ToString())
                            {
                                ...
                                case "required":
                                    ...
                                    {
                                        msg[name.ToString()] =  (JsonData)this.decodeProp(type.ToString(), proto);
